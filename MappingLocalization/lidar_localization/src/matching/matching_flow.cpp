...
MatchingFlow::MatchingFlow(ros::NodeHandle& nh) :nh_(nh) {
    cloud_sub_ptr_ = std::make_shared<CloudSubscriber>(nh, "/velodyne_points", 1);
    gnss_sub_ptr_ = std::make_shared<OdometrySubscriber>(nh, "/gps/odometry", 100);
...
void MatchingFlow::Trans_lidar2gps(const Eigen::Matrix4f& pose, Eigen::Matrix4f& respose){
    ...
    float x_lidar_in_baselink(0.25);
    float y_lidar_in_baselink(0);
    float angle_lidar_in_baselink(0);

    Eigen::Affine3f T_LG = pcl::getTransformation(-x_lidar_in_baselink, -y_lidar_in_baselink, 0, 0, 0, -angle_lidar_in_baselink);
...
{   
    ...
    if(matching_loc_enable==false)  
    {
        matching_loc_enable=true;
...
void MatchingFlow::clearState()
{
    ...
    nh_.setParam("matching_loc_enable", matching_loc_enable);
}
